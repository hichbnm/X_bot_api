...
// ---------------- MAIN FUNCTION ----------------
func main() {
	http.HandleFunc("/post-tweet", handlePostTweet)

	fmt.Println("🚀 API running on http://localhost:8099")
	http.ListenAndServe(":8099", nil)
}

// Read auth_token from auth_token.txt
...
func handlePostTweet(w http.ResponseWriter, r *http.Request) {
	...
	// Post tweet
	resp := postWithParams(authToken, ct0, bearerToken, req.TweetText, guest_id)
...
func postWithParams(authToken, xCsrfToken, bearerToken, tweetText, guestID string) string {
